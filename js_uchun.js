  ...
  try {
    ...
    if (link.includes("youtube.com") || link.includes("youtu.be")) {
      if (data.thumbnail) {
        ...
        appendMessage("❌ Thumbnail topilmadi.", "bot");
      }
      return;
    }
...
function appendBotImage(url, medias = [], originalUrl = "") {
  ...
  if (medias.length > 0) {
    ...
    const filtered = medias.filter(m => {
      if (seen.has(m.quality)) return false;
      seen.add(m.quality);
      return true;
    });

    filtered.sort((a, b) => parseInt(a.quality) - parseInt(b.quality));
    ...
    filtered.forEach(media => {
      ...
      btn.textContent = media.quality || "Download";
      btn.className = "btn btn-primary";
      btn.style.width = "100%";

      btn.onclick = async () => {
        wrapper.remove(); // ❌ eski rasm va tugmalarni o‘chirish

        appendMessage("⏳ Video tayyorlanmoqda...", "bot");
        ...
        const data = await res.json();
        if (data.url) {
          appendMessage(`<a href="${data.url}" target="_blank">📥 ${media.quality} videoni yuklab olish</a>`, "bot");
        } else {
          appendMessage("❌ Video tayyorlanmadi!", "bot");
        }
      };
